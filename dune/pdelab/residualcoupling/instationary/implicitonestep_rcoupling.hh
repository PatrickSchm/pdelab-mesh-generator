...
#ifndef DUNE_PDELAB_INSTATIONARY_IMPLICITONESTEP_RCOUPLING_HH
#define DUNE_PDELAB_INSTATIONARY_IMPLICITONESTEP_RCOUPLING_HH
...
struct OneStepMethodPartialResultCoupling
{
  ...
  OneStepMethodPartialResultCoupling() :
    timesteps(0), assembler_time(0.0), linear_solver_time(0.0), linear_solver_iterations(
      0), nonlinear_solver_iterations(0)
  {
  }
};

struct OneStepMethodResultCoupling
{
  OneStepMethodPartialResultCoupling total;
  OneStepMethodPartialResultCoupling successful;
  OneStepMethodResultCoupling() :
    total(), successful()
  {
  }
};
 ...
 */
template<class T, class IGOS, class PDESOLVER, class TrlV, class TrlVSL,
         class TstV = TrlV>
class OneStepMethodCoupling
{
  typedef typename PDESOLVER::Result PDESolverResult;

public:
  typedef OneStepMethodResultCoupling Result;
   ...
   * there).
   */
  OneStepMethodCoupling(const TimeSteppingParameterInterface<T>& method_,
                        IGOS& igos_, PDESOLVER& pdesolver_) :
    method(&method_), igos(igos_), pdesolver(pdesolver_), verbosityLevel(
      1), step(1), res()
  {
  ...
  T apply(T time, T dt, TrlV& xold, TrlV& xnew)
  {
    ...
    for (unsigned r = 1; r <= method->s(); ++r)
    {
      ...
      if (r == method->s())
      {
        ...
        x.push_back(&xnew);
        if (r > 1)
          xnew = *(x[r - 1]); // if r=1 then xnew has already initial guess
      } else
      {
        // intermediate step
  ...
  template<typename F>
  T applyCoupling(T time, T dt, TrlV& xold, F& f, TrlV& xnew, TrlVSL& xSlold)
  {
    // do statistics
    OneStepMethodPartialResultCoupling step_result;
    auto XOLD = xnew;
