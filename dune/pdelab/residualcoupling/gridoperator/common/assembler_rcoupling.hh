 ...
 */
class LocalAssemblerEngineRCoupling
{
public:
    ...
    void assembleCBoundary(const IG& ig, const EGC& egC, const LFSU& lfsu,
...
//           @{
//           */
    void onBindLFSUV(const EG & eg, const LFSU_S & lfsu_s,
                     const LFSV_S & lfsv_s);
    void onBindLFSV(const EG & eg, const LFSV_S & lfsv_s);
    void onBindLFSUVInside(const IG & ig, const LFSU_S & lfsu_s,
                           const LFSV_S & lfsv_s);
    ...
    void onBindLFSVOutside(const IG & ig, const LFSV_S & lfsv_s,
                           const LFSV_N & lfsv_n); void onBindLFSUVCoupling(const IG & ig,
                                                                            const LFSU_S & lfsu_s, const LFSV_S & lfsv_s,
                                                                            const LFSU_N & lfsu_n, const LFSV_N & lfsv_n
    ...
    void onBindLFSVCoupling(const IG & ig, const LFSV_S & lfsv_s,
                            const LFSV_N & lfsv_n, const LFSV_Coupling & lfsv_coupling);

    void onUnbindLFSUV(const EG & eg, const LFSU_S & lfsu_s,
                       const LFSV_S & lfsv_s);
    void onUnbindLFSUVC(const EGSL & egSl, const LFSU_SSL & lfsu_sSl,
                        const LFSV_SSL & lfsv_sSl);
    void onUnbindLFSVC(const EGSL & egSl, const LFSV_SSL & lfsv_sSl);
    void onUnbindLFSV(const EG & eg, const LFSV_S & lfsv_s);
    void onUnbindLFSUVInside(const IG & ig, const LFSU_S & lfsu_s,
                             const LFSV_S & lfsv_s);
    ...
    void loadCoefficientsLFSUCoupling(
    ...
    void setSolutionOld(const X& xOld);
    void setSolutionSl(const XSl& xSl);
...
public:
    ...
    LocalResidualAssemblerEngine & localResidualAssemblerEngine(R & r,
                                                                const X & x);
    LocalResidualAssemblerEngineCoupling & localResidualAssemblerEngineCoupling(
        R & r, const X & x, const XSl & xSl);
    LocalJacobianAssemblerEngine & localJacobianAssemblerEngine(A & a,
                                                                const X & x);
    LocalResidualJacobianAssemblerEngine & localResidualJacobianAssemblerEngine(
        R & r, A & a, const X & x);
    /** @} */
 ...
 */
template<typename GFSU, typename GFSV, typename MB, typename DF,
...
public:
    ...
    //! The traits class
    typedef GridOperatorTraits<GFSU, GFSV, MB, DF, RF, JF, CU, CV,
            AssemblerInterface, LocalAssemblerInterface> Traits;
     ...
     */
    template<typename F>
    void interpolate(const typename Traits::Domain& xold, const F& f,
