#ifndef DUNE_PDELAB_DEFAULT_ASSEMBLERSL_RCOUPLING_HH
#define DUNE_PDELAB_DEFAULT_ASSEMBLERSL_RCOUPLING_HH
 ...
 */
template<typename ...> class DefaultAssemblerCouplingSl;
...
class DefaultAssemblerCouplingSl<GFSUMA, GFSVMA, GFSUC, GFSVC, CU, CV,
...
public:
    ...
    DefaultAssemblerCouplingSl(const GFSUMA& gfsuMa_,
    ...
    void assemble(LocalAssemblerEngine & assembler_engine) const
    {
        ...
        for (const auto& element : elements(entity_set))
        {
            ...
            // const bool requireCoupling = couplingInterface[idsindex].cut;
            ...
            // auto cBegin = couplingInterface[idsindex].coupledElements.begin();
            // auto cEnd = couplingInterface[idsindex].coupledElements.end();

            // ElementGeometry<ElementC> egC(*cBegin);
            // lfsvC.bind(*cBegin);
            // lfsv_cacheC.update();
            // assembler_engine.onBindLFSVSL(egC, lfsv_cacheC);
            // lfsuC.bind(*cBegin);
            // lfsu_cacheC.update();
            ...
            // for (const auto& intersection : intersections(entity_setC, *cBegin))
            // {
            //     if (intersection.boundary()) {
            //         const ReferenceElement <SizeType, 2 > &refTriangle
            //             = ReferenceElements<SizeType, 2 >::simplex();
            //         auto geometryInside = intersection.geometryInInside();
            //         auto intersectionIndex = intersection.indexInInside();
            //         int nodeLocalIndex0 = refTriangle.subEntity(intersectionIndex, 1, 0, 2);
            //         int nodeLocalIndex1 = refTriangle.subEntity(intersectionIndex, 1, 1, 2);

            //         auto geoC = egC.geometry();

            //         int nodeGlobalIndex0 = index_setC.subIndex(*cBegin, nodeLocalIndex0, 2);
            //         int nodeGlobalIndex1 = index_setC.subIndex(*cBegin, nodeLocalIndex1, 2);

            //         // fractureHeight.push_back(aligningNodes[nodeGlobalIndex0]);
            //         // fractureHeight.push_back(aligningNodes[nodeGlobalIndex1]);

            //         IntersectionGeometry<IntersectionC> igC(intersection, intersection_index);
            //         // Boundary integration

                    // assembler_engine.assembleCBoundaryReversed(eg, igC, lfsu_cacheMa, lfsv_cacheMa, lfsu_cacheC, lfsv_cacheC, fractureHeight);
            //     }
            //     ++intersection_index;
            // }
