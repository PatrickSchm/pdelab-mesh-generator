#ifndef DUNE_PDELAB_DEFAULT_PATTERNENGINE_COUPLING_RCOUPLING_HH
#define DUNE_PDELAB_DEFAULT_PATTERNENGINE_COUPLING_RCOUPLING_HH
...
class DefaultLocalPatternAssemblerEngineCouplingRCoupling: public LocalAssemblerEngineBaseRCoupling
{
public:
  ...
  /**
   ...
   creates this engine
   */
  DefaultLocalPatternAssemblerEngineCouplingRCoupling(
    const LocalAssembler & local_assembler_,
    shared_ptr<typename LA::Traits::BorderDOFExchanger> border_dof_exchanger) :
    local_assembler(local_assembler_), lop(local_assembler.lop), pattern(
      nullptr), _border_dof_exchanger(
        border_dof_exchanger)
  {
  }
  ...
  {
    return local_assembler.doPatternCoupling();
  }
  ...
  void add_border_pattern(std::true_type, const LFSVC& lfsv_cache,
                          const LFSUC& lfsu_cache, const LocalPattern& p)
  {
  ...
  template<typename LFSVC, typename LFSUC>
  void add_border_pattern(std::false_type, const LFSVC& lfsv_cache,
                          const LFSUC& lfsu_cache, const LocalPattern& p)
  {
  }

  template<typename LFSVC, typename LFSUC>
  void add_pattern(const LFSVC& lfsv_cache, const LFSUC& lfsu_cache,
                   const LocalPattern& p)
  {
    for (size_type k = 0; k < p.size(); ++k)
      local_assembler.add_entry(*pattern, lfsv_cache, p[k].i(),
                                lfsu_cache, p[k].j());

    add_border_pattern(
      std::integral_constant<bool,
      LocalAssembler::isNonOverlapping>(), lfsv_cache,
      lfsu_cache, p);
  }
                        ...
                        const LFSVC & lfsv_cache, int iMat)
  {
    Dune::PDELab::LocalAssemblerCallSwitchRCoupling<LOP,
         LOP::doPatternVolume>::pattern_volume(lop,
                                               lfsu_cache.localFunctionSpace(),
                                               lfsv_cache.localFunctionSpace(), localpattern);
  }

  template<typename IG, typename EGC, typename LFSVC, typename LFSUCC, typename LFSVCC>
  void assembleCBoundary(const IG& ig, const EGC& egC,
                         const LFSVC& lfsv_cache, const LFSUCC& lfsuC_cache,
                         const LFSVCC& lfsvC_cache)
  {
    Dune::PDELab::LocalAssemblerCallSwitchRCoupling<LOP,
         LOP::doPatternCoupling>::pattern_coupling(lop,
                                                   lfsv_cache.localFunctionSpace(),
                                                   lfsuC_cache.localFunctionSpace(),
  ...
  void assembleCBoundaryReversed(const IG& ig, const EGC& egC, const LFSUC& lfsu_cache,
                                 const LFSVC& lfsv_cache, const LFSUCC& lfsuC_cache,
                                 const LFSVCC& lfsvC_cache)
  {
    Dune::PDELab::LocalAssemblerCallSwitchRCoupling<LOP,
         LOP::doPatternCoupling>::pattern_coupling_reversed(lop,
                                                   lfsu_cache.localFunctionSpace(),
                                                   lfsv_cache.localFunctionSpace(),
                                                   lfsuC_cache.localFunctionSpace(),
                                                   lfsvC_cache.localFunctionSpace(),
  ...
  {
    Dune::PDELab::LocalAssemblerCallSwitchRCoupling<LOP,
         LOP::doPatternSkeleton>::pattern_skeleton(lop,
                                                   lfsu_s_cache.localFunctionSpace(),
                                                   lfsv_s_cache.localFunctionSpace(),
                                                   lfsu_n_cache.localFunctionSpace(),
                                                   lfsv_n_cache.localFunctionSpace(), localpattern_sn,
                                                   localpattern_ns);
  }
  ...
  {
    Dune::PDELab::LocalAssemblerCallSwitchRCoupling<LOP,
         LOP::doPatternBoundary>::pattern_boundary(lop,
                                                   lfsu_s_cache.localFunctionSpace(),
                                                   lfsv_s_cache.localFunctionSpace(), localpattern);
  }
  ...
  {
    Dune::PDELab::LocalAssemblerCallSwitchRCoupling<LOP,
         LOP::doPatternVolumePostSkeleton>::pattern_volume_post_skeleton(
           lop, lfsu_cache.localFunctionSpace(),
           lfsv_cache.localFunctionSpace(), localpattern);
  }

  void postAssembly(const GFSU& gfsu, const GFSV& gfsv)
  {
    post_border_pattern_assembly(
      std::integral_constant<bool,
      LocalAssembler::isNonOverlapping>(), gfsu,
      gfsv);
  }

  void postAssemblyCoupling(const GFSU& gfsu, const GFSV& gfsv,
                            const GFSUSL& gfsuSl, const GFSVSL& gfsvSl)
  {
    ...
    post_border_pattern_assembly_coupling(
      std::integral_constant<bool,
      LocalAssembler::isNonOverlapping>(), gfsuSl,
      gfsvSl);
  }

  void post_border_pattern_assembly(std::true_type, const GFSU& gfsu,
                                    const GFSV& gfsv)
  {
        ...
        && local_assembler.reconstructBorderEntries())
    {
      ...
      typename LA::Traits::BorderDOFExchanger::template PatternExtender <
        Pattern > data_handle(*_border_dof_exchanger, gfsu,
                              gfsv, *pattern);
  ...
  void post_border_pattern_assembly_coupling(std::true_type,
                                             const GFSUSL& gfsuSl, const GFSVSL& gfsvSl)
  {
    if (local_assembler.doPostProcessing
        && local_assembler.reconstructBorderEntries())
    {
      ...
      typename LA::Traits::BorderDOFExchanger::template PatternExtender <
        Pattern > data_handle(*_border_dof_exchanger, gfsuSl,
                              gfsvSl, *pattern);
      gfsvSl.gridView().communicate(data_handle,
                                    InteriorBorder_InteriorBorder_Interface,
                                    ForwardCommunication);
    }
  }

  void post_border_pattern_assembly(std::false_type, const GFSU& gfsu,
                                    const GFSV& gfsv)
  {
  }
