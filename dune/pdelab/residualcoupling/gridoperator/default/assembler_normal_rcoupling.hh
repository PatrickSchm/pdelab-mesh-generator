#ifndef DUNE_PDELAB_DEFAULT_ASSEMBLER_NORMAL_RCOUPLING_HH
#define DUNE_PDELAB_DEFAULT_ASSEMBLER_NORMAL_RCOUPLING_HH
...
class DefaultAssemblerNormalRcoupling {
public:
  ...
  static const bool isGalerkinMethod = is_same<GFSU, GFSV>::value;

  DefaultAssemblerNormalRcoupling(const GFSU& gfsu_, const GFSV& gfsv_, const CU& cu_, const CV& cv_)
    : gfsu(gfsu_)
    , gfsv(gfsv_)
    , cu(cu_)
    , cv(cv_)
  ...
  DefaultAssemblerNormalRcoupling(const GFSU& gfsu_, const GFSV& gfsv_)
    : gfsu(gfsu_)
    , gfsv(gfsv_)
    , cu()
    , cv()
