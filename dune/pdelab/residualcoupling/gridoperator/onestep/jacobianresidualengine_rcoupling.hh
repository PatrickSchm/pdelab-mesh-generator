#ifndef DUNE_PDELAB_ONESTEP_JACOBIANRESIDUALENGINE_RCOUPLING_HH
#define DUNE_PDELAB_ONESTEP_JACOBIANRESIDUALENGINE_RCOUPLING_HH
...
class OneStepExplicitLocalJacobianResidualAssemblerEngineCoupling
    : public LocalAssemblerEngineBaseRCoupling
{
public:
    //! The type of the wrapping local assembler
    typedef OSLA OneStepLocalAssemblerCoupling;
    ...
    typedef typename LocalAssembler::LocalAssemblerCouplingDT1::LocalJacobianAssemblerEngine JacobianEngine;
    ...
    /**
     ...
     creates this engine
    */
    OneStepExplicitLocalJacobianResidualAssemblerEngineCoupling
    ...
    bool requireBoundaryCoupling() const
    {    return prestage_engine->requireBoundaryCoupling() ||
               jacobian_engine->requireBoundaryCoupling();
    }
    bool requireBoundaryCouplingReversed() const
    {    return prestage_engine->requireBoundaryCouplingReversed() ||
               jacobian_engine->requireBoundaryCouplingReversed();
    }
    ...
    {
        if (prestage_engine->requireUVVolume())
            prestage_engine->assembleUVVolume(eg, lfsu, lfsv, iMat);
        la.setWeight(-1.0);
        prestage_engine->setTimeInLastStage();
        if (jacobian_engine->requireUVVolume())
            jacobian_engine->assembleUVVolume(eg, lfsu, lfsv, iMat);
    }

    template<typename IG, typename LFSV>
    void assembleCBoundary(const IG & ig,  const LFSV & lfsv_s)
    {
        if (prestage_engine->requireBoundaryCoupling())
            prestage_engine->assembleCBoundary(ig, lfsv_s);
        la.setWeight(-1.0);
        if (jacobian_engine->requireBoundaryCoupling())
            jacobian_engine->assembleCBoundary(ig, lfsv_s);
    }

    template<typename IG, typename LFSU, typename LFSV>
    void assembleCBoundaryReversed(const IG & ig, const LFSU & lfsu, const LFSV & lfsv)
    {
        if (prestage_engine->requireBoundaryCouplingReversed())
            prestage_engine->assembleCBoundaryReversed(ig, lfsu, lfsv);
        la.setWeight(-1.0);
        prestage_engine->setTimeInLastStage();
        if (jacobian_engine->requireBoundaryCouplingReversed()) {

            jacobian_engine->assembleCBoundaryReversed(ig, lfsu, lfsv);
        }
    }
