...
struct StationaryLinearProblemSolverCouplingResultCoupling: LinearSolverResult<RFType>
{
    ...
    StationaryLinearProblemSolverCouplingResultCoupling() :
        first_defect(0.0), defect(0.0), assembler_time(0.0), linear_solver_time(
            0.0), linear_solver_iterations(0)
    {
    }

};

template<typename GO, typename LS, typename V, typename VSL>
class StationaryLinearProblemSolverCoupling
{
...
public:
    typedef StationaryLinearProblemSolverCouplingResultCoupling<double> Result;

    StationaryLinearProblemSolverCoupling(const GO& go, LS& ls, V& x, VSL& xSl,
                                          Real reduction, Real min_defect = 1e-99, int verbose = 1) :
        _go(go), _ls(ls), _x(&x), _xSl(&xSl), _reduction(reduction), _min_defect(
            min_defect), _hanging_node_modifications(false), _keep_matrix(
                true), _verbose(verbose)
    {
    }

    StationaryLinearProblemSolverCoupling(const GO& go, LS& ls, Real reduction,
                                          Real min_defect = 1e-99, int verbose = 1) :
        _go(go), _ls(ls), _x(), _reduction(reduction), _min_defect(
            min_defect), _hanging_node_modifications(false), _keep_matrix(
                true), _verbose(verbose)
    {
    }

    //! Construct a StationaryLinearProblemSolverCoupling for the given objects and read parameters from a ParameterTree.
    /**
    ...
    StationaryLinearProblemSolverCoupling(const GO& go, LS& ls, V& x, VSL& xSl,
        ...
        _go(go), _ls(ls), _x(&x), _xSl(&xSl), _reduction(
            params.get<Real>("reduction")), _min_defect(
                params.get<Real>("min_defect", 1e-99)), _hanging_node_modifications(
                    params.get<bool>("hanging_node_modifications",
                                     false)), _keep_matrix(
                                         params.get<bool>("keep_matrix", true)), _verbose(
                                             params.get<int>("verbosity", 1))
    {
    }
     ...
     * where start defect is the norm of the residual of x.
     */
    StationaryLinearProblemSolverCoupling(const GO& go, LS& ls,
                                          const ParameterTree& params) :
        _go(go), _ls(ls), _x(), _xSl(), _reduction(
            params.get<Real>("reduction")), _min_defect(
                params.get<Real>("min_defect", 1e-99)), _hanging_node_modifications(
                    params.get<bool>("hanging_node_modifications",
                                     false)), _keep_matrix(
    ...
    void apply(V& x, V& xOld, VSL& xSl, bool reuse_matrix = false)
    {
        _x = &x;
        _xSl = &xSl;
        _xOld = &xOld;
    ...
    void apply(bool reuse_matrix = false)
    {
        ...
        if (!_jacobian)
        {
            ...
            if (_go.trialGridFunctionSpace().gridView().comm().rank()
                    == 0 && _verbose >= 1)
                std::cout << "=== matrix setup (max) " << timing << " s"
                          << std::endl;
            watch.reset();
            assembler_time += timing;
        } else if (_go.trialGridFunctionSpace().gridView().comm().rank()
                   == 0 && _verbose >= 1)
            std::cout
        ...
        {
            Dune::PDELab::set_shifted_dofs(
                _go.localAssembler().trialConstraints(), 0.0, *_x); // set hanging node DOFs to zero
        ...
        {
            (*_jacobian) = Real(0.0);
            _go.jacobian(*_x, *_xOld, *_xSl, *_jacobian);
        }
                ...
                && _verbose >= 1)
        {
            if (reuse_matrix)
                ...
                std::cout << "=== matrix assembly (max) " << timing
                          << " s" << std::endl;
        }
        ...
        _go.residual(*_x, *_xOld, *_xSl, r);  // residual is additive

        timing = watch.elapsed();
        // timing = gos.trialGridFunctionSpace().gridView().comm().max(timing);
        if (_go.trialGridFunctionSpace().gridView().comm().rank() == 0
                && _verbose >= 1)
            std::cout << "=== residual assembly (max) " << timing
                      << " s" << std::endl;
        assembler_time += timing;
        ...
        _res.defect = static_cast<double>(defect)
                      * _linear_solver_result.reduction;
        _res.linear_solver_iterations =
            _linear_solver_result.iterations;
