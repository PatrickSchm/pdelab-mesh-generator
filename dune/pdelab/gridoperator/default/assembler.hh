...
template<typename GFSU, typename GFSV, typename CU, typename CV, typename MaterialFlags, bool nonoverlapping_mode = false>
...
public:
  ...
  DefaultAssembler (const GFSU& gfsu_, const GFSV& gfsv_, const CU& cu_, const CV& cv_, const MaterialFlags & materialFlags_)
    : gfsu(gfsu_)
    , gfsv(gfsv_)
    , cu(cu_)
    , cv(cv_)
  ...
  void assemble(LocalAssemblerEngine & assembler_engine) const
  {
    ...
    for (const auto& element : elements(entity_set))
    {
      ...
      // Volume integration
      assembler_engine.assembleUVVolume(eg, lfsu_cache, lfsv_cache, 0);
