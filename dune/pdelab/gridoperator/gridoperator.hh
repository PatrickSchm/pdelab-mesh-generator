...
"The correct mode is now automatically deduced from the EntitySet of the function space.")
    ...
    */
    template<typename GFSU, typename GFSV, typename LOP,
             typename MB, typename DF, typename RF, typename JF, typename MaterialFlags,
    ...
    public:
      ...
      typedef DefaultAssembler<GFSU,GFSV,CU,CV,MaterialFlags, static_cast<bool>(nonoverlapping_mode)> Assembler;
      ...
      GridOperator(const GFSU & gfsu_, const CU & cu_, const GFSV & gfsv_, const CV & cv_, LOP & lop_, const MaterialFlags & materialFlags_, const MB& mb_ = MB())
        : global_assembler(gfsu_,gfsv_,cu_,cv_, materialFlags_)
        , dof_exchanger(std::make_shared<BorderDOFExchanger>(*this))
        , local_assembler(lop_, cu_, cv_,dof_exchanger)
        , backend(mb_)
        , materialFlags(materialFlags_)
      {}

      //! Constructor for empty constraints
      GridOperator(const GFSU & gfsu_, const GFSV & gfsv_, LOP & lop_, const MaterialFlags & materialFlags_, const MB& mb_ = MB())
